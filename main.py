...
def show_all(*args):
    if not phonebook:
        raise ValueError('No contacts')                      # Викликаємо помилку, якщо немає записів
    result = ''
    chunk_size = 7
    user_page = None
    if len(args) > 1:
...
COMMANDS = {'remove': remove,            # Видаляє номер у контакта
            'add': add,                  # Додає новий контакт або якщо вже є додає номер або дату народження.
            'change': change,            # Змінює старий номер на новий
            'phone': phone,              # Пошук номеру(номерів) телефону контакту за ім'ям
            'show all': show_all,        # Виводить всі записи по сторінково, або одну сторінку
